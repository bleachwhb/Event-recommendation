...
//--------------------		
		if(item.localDate) {
			var local_time = $('p', {
				className : 'item-local_time'
			});
			local_time.innerHTML = 'Events local time: ' + item.localDate;
			if(item.localTim) {
				local_time.innerHTML = local_time.innerHTML + " " + item.localTim.replace("undifined", "");
			}
			section.appendChild(local_time);
		}
		if(item.localTim) {
			var test = $('p', {
				className : 'item-duration'
			});
			test.innerHTML = 'driving-duration: ' + item.test;
		...
		if(item.duration) {
			...
			duration.innerHTML = 'driving-duration: ' + item.duration.replace("?","");
		...
		if(item.distance) {
			...
			distance.innerHTML = 'driving-distance: ' + item.distance.replace("?","");
		...
		if(item.min && item.max) {
			...
			price_range.innerHTML = 'price_range ' + item.min + "—" + item.max;
			section.appendChild(price_range);
		}
